package com.cutsquash.objloader2;
/*
 * .obj loader for processing
 * written by Justin Pinkney 15/04/2015
 * based on objloader by Tatsuya SAITO / UCLA Design | Media Arts 
 * 
 */
...
public class OBJModel2 {
  ...
  public void enableDebug() {
    debug.enabled = true;
    debug.println("");
    debug.println("New OBJ loader");
    debug.println("2015");
    debug.println("Based on: http://code.google.com/p/saitoobjloader/");
    debug.println("");
  }
   ...
   * </br> <br>
   * </br> OBJModel2 model = new OBJModel2(this, "myobj.obj");<br>
   ...
   * </br> OBJModel2 model = new OBJModel2(this, "myobj.obj");<br>
   * </br> // do something with model<br>
   * </br> model = new OBJModel2(this, "myOtherObj.obj");<br>
   * </br> <br>
  ...
  public void reset() {
    ...
    materials.clear();
    originalTexture();
    debug.println("OBJModel2 is empty");
  }
  ...
  // -------------------------------------------------------------------------
  /**
   * The draw method of the OBJModel2.<br>
  ...
  /**
   * Convert the model to a single PShape
   */

  public PShape convertModel() {

    PShape shape = parent.createShape(PConstants.GROUP);
    ...
    try {
      ...
      // for each segment generate a child shape (one of tris, and one of quads)
      for (int s = 0; s < getSegmentCount(); s++) {

        // set up the two possible shapes for this segment
        PShape triShape = parent.createShape();
        PShape quadShape = parent.createShape();

        triShape.beginShape(PConstants.TRIANGLES);
        quadShape.beginShape(PConstants.QUADS);
        ...
        if (useMaterial) {
          triShape.ambient(255.0f * tmpMaterial.Ka[0], 255.0f * tmpMaterial.Ka[1], 255.0f * tmpMaterial.Ka[2]);
          triShape.specular(255.0f * tmpMaterial.Ks[0], 255.0f * tmpMaterial.Ks[1], 255.0f * tmpMaterial.Ks[2]);
          
          quadShape.ambient(255.0f * tmpMaterial.Ka[0], 255.0f * tmpMaterial.Ka[1], 255.0f * tmpMaterial.Ka[2]);
          quadShape.specular(255.0f * tmpMaterial.Ks[0], 255.0f * tmpMaterial.Ks[1], 255.0f * tmpMaterial.Ks[2]);
        ...
        for (int f = 0; f < tmpModelSegment.getFaceCount(); f++) {
          ...
          if (tmpModelElement.getVertIndexCount() > 0) {
            ...
            triShape.textureMode(PConstants.NORMAL);
            quadShape.textureMode(PConstants.NORMAL);
            if (useTexture == false || tmpMaterial.map_Kd == null)
              useTexture = false;
              
            // TODO need to load the right texture
            if (useTexture) {

                triShape.texture(tmpMaterial.map_Kd); 
                quadShape.texture(tmpMaterial.map_Kd);

            }

            // If this face is a triangle:
            if (tmpModelElement.getVertIndexCount() == 3){
              ...
              for (int fp = 0; fp < tmpModelElement.getVertIndexCount(); fp++) {
                ...
                if (v != null) {
                  try {
                    if (tmpModelElement.normalIndices.size() > 0) {
                      vn = normalVertices.get(tmpModelElement.getNormalIndex(fp));
                      triShape.normal(vn.x, vn.y, vn.z);
                    }

                    if (useTexture) {
                      vt = textureVertices.get(tmpModelElement.getTextureIndex(fp));
                      triShape.vertex(v.x, v.y, v.z, vt.x, vt.y);
                    } else
                      triShape.vertex(v.x, v.y, v.z);
                  } catch (Exception e) {
                    e.printStackTrace();
                  }
                } else
            ...
            } else if (tmpModelElement.getVertIndexCount() == 4){
              ...
              for (int fp = 0; fp < tmpModelElement.getVertIndexCount(); fp++) {
                ...
                if (v != null) {
                  try {
                    if (tmpModelElement.normalIndices.size() > 0) {
                      vn = normalVertices.get(tmpModelElement.getNormalIndex(fp));
                      quadShape.normal(vn.x, vn.y, vn.z);
                    }

                    if (useTexture) {
                      vt = textureVertices.get(tmpModelElement.getTextureIndex(fp));
                      quadShape.vertex(v.x, v.y, v.z, vt.x, vt.y);
                    } else
                      quadShape.vertex(v.x, v.y, v.z);
                  } catch (Exception e) {
                    e.printStackTrace();
                  }
                } else
